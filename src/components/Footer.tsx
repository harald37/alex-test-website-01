  ...
  return (
    <footer className="w-full border-t border-outline-variant/10 bg-surface">
      <div className="flex flex-col md:flex-row justify-between items-center w-full px-12 py-16 gap-8 max-w-screen-2xl mx-auto">
        <div className="font-serif italic text-xl text-zinc-900">
          Alex Begopoulos
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-x-12 gap-y-4">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.name}
              to={link.path}
              className="tracking-[0.1em] text-[10px] uppercase text-zinc-400 hover:text-zinc-900 underline-offset-4 hover:underline transition-all"
            >
              {link.name}
            </Link>
          ))}
        ...
        <div className="tracking-[0.1em] text-[10px] uppercase text-zinc-400">
